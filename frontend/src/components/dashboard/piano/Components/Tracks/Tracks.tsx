...
export default function Tracks({Data,Speed,Width,Height, BlackNumbers, KeysPositions,intervalSpeed,options,Player,sound}:TracksProps):ReactElement {
    ...
    const drawPianoKeys = (arr:Array<blockNote>) =>{
        arr.map(element =>{
            if(pianoCtx){
                // const pos_x = KeysPositions[element.NoteNumber - 48].position; // Adjust the offset to 48 (C3) for note number
                ...
                const width = BlackNumbers.includes(element.NoteNumber) ? Width / 14 / 1.8 : Width / 14;
                ...
                if(element.wasDetected){
                    ...
                    // sound && sound.play_key(element.NoteNumber-48,element.duration,element.Velocity); // for playing the sound from C3
                }
                else{
                    if(BlackNumbers.includes(element.NoteNumber)){
    ...
    return (
        <div>
            ...
            <canvas ref={tracksRef} width={Width.toString() + 'px'} height={(Height - Height/5).toString() + 'px'} className='Canvas'></canvas> {/*to draw the tracks where the piano notes fall*/}
            <canvas ref={EffectsRef} width={Width} height={Height - Height/5} className='Effects'></canvas> {/* to have the visual effects */}
            <canvas ref={GradeintRef} width={Width} height={Height - Height/5 + 50} className='Gradient'></canvas> {/* to have the gradient effect */}
            </>}
            ...
            <Piano wh={fixedHeight - fixedHeight/1.5} // /1.2 for the piano to be in the Card container of Play.tsx
             WhiteKeyWidth={fixedWidth/14} height={fixedHeight/2} data={[]} sound={sound} /> {/*todo: change the 分母 if piano not well displayed*/}
            ...
            <canvas ref={PianoRef} width={fixedWidth} height={fixedHeight/5 + 5} style={{position:'absolute',zIndex:34,marginTop:fixedHeight - fixedHeight/5}} /> {/*to draw the piano keys when pressed*/}
            <canvas ref={PianoWhiteRef} width={fixedWidth} height={fixedHeight/5 + 5} style={{position:'absolute',zIndex:32,marginTop:fixedHeight - fixedHeight/5}} /> {/*to draw the piano white keys when pressed*/}
        </div>
    )
}
