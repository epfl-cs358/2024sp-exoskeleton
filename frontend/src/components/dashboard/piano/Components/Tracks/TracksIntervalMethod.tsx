...
export default function TracksIntervalMethod({Data,Speed, BlackNumbers, KeysPositions,intervalSpeed,options,Player,sound}:TracksProps):ReactElement {
        ...
        if(!Player.isPaused){
            ...
            blocksToMap.reverse().map(block =>{
                ...
                if(block.pos_y > Height-Height/5){
                    const gradient = context!.createRadialGradient(block.pos_x + Width / 14 / 2, Height, 15, block.pos_x + Width / 14 / 2, Height, 45);
                    ...
                    context!.fillRect(block.pos_x + Width / 14 / 2 - 50, Height - 50,block.pos_x + Width / 14 / 2 +50, Height +50);
    ...
    useEffect(() =>{
        ...
        Data && Data.map(Event =>{
            const newBlock:blockNote = {
                ...
                width: BlackNumbers.includes(Event.NoteNumber) ? Width / 14 / 1.8 : Width / 14,
                Velocity: Event.Velocity,
                NoteNumber: Event.NoteNumber,
                // pos_x: KeysPositions[Event.NoteNumber - 48].position,
    ...
    return (
        <div>
            ...
            <Piano wh={Height - Height/5} height={Height/5} WhiteKeyWidth={Width / 14} data={keysNotes} sound={sound} />
        </div>
    )
}